	...
	Tree.prototype = {
			...
			this.options.dataSource.data($el.data(), function(items) {
				...
				$.each(items.data, function(index, value) {
					...
					if ($el.hasClass('tree-folder-header')) {
						$el.parent().find('.tree-folder-content:eq(0)').append(
								$entity);
					} else {
						$el.append($entity);
					}
		...
		selectItem : function(el) {
			...
			if (this.options.multiSelect) {
				...
				$all.removeClass('tree-selected').find('i').removeClass(
						'icon-ok').addClass('tree-dot');
				data.push($el.data());
			}
		...
		selectFolder : function(el) {
			...
			if ($el.find('.icon-folder-close').length) {
				...
				$par.find('.icon-folder-close:eq(0)').removeClass(
				...
				$par.find('.icon-folder-open:eq(0)').removeClass(
						'icon-folder-open').addClass('icon-folder-close');
